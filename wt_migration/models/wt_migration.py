...
class TaskMigration(models.Model):
    ...
    def create(self, values):
        ...
        self.env['wt.field.map'].create_template(migration)
        return migration
    ...
    def load_clone_template(self, destination_migration, clone_rule):
        ...
        if rule:
            fields = {
                fr.field_id: fr.template for fr in rule.clone_field_ids
            }
            types = dict()
    ...
    def get_local_issue_data(self, domain=[]):
        return {
            ...
            'dict_type': {(r.key, r.company_id.id): r.id for r in self.env["wt.type"].sudo().with_context(active_test=False).search([])},
            ...
            'dict_field_map': {r.key: r.value for r in self.env['wt.field.map'].sudo().search([('migration_id', '=', self.id)])}
        }
    ...
    def create_missing_statuses(self, issues, local):
        ...
        for issue in issues:
            if (issue.remote_status_id, company.id) not in local['dict_status']:
                local['dict_status'][(issue.remote_status_id, company.id)] = self.env['wt.status'].sudo().create({
    ...
    def create_missing_types(self, issues, local):
        ...
        for issue in issues:
            if (issue.issue_type_key, company.id) not in local['dict_type']:
                local['dict_type'][(issue.issue_type_key, company.id)] = self.env['wt.type'].sudo().create({
    ...
    def create_missing_labels(self, issues, local):
        ...
        for label in set_labels:
            if (label, company.id) not in local['dict_label']:
                res = self.env['wt.label'].sudo().create({
                    'name': label,
                    'company_id': company.id
                })
